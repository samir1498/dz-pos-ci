...
import { TillScreen } from "../../src/routes/till";
...
describe("the quantity a line carries", () => {
  ...
  test.each([
    ["the piece", coffee],
    ["the box", crate],
  ])("a product sold by %s refuses 1,5 and nothing is posted", async (_unit, product) => {
    const user = userEvent.setup();
    mount();
    await findTile(product);
    await user.click(tile(product));
    const qty = screen.getByLabelText(`Quantité ${product.name}`);
    await user.clear(qty);
    await user.type(qty, "1,5");
...
describe("the discounts the screen refuses on its own", () => {
  test.each([
    ...
    ["Remise ligne (DA) " + coffee.name, "400,01", "La remise dépasse la ligne."],
    ["Remise globale (DA)", "500", "La remise dépasse le panier."],
  ])("a discount typed into %j never reaches the API", async (label, value, message) => {
    const user = userEvent.setup();
    mount();
    await findTile(coffee);
    await user.click(tile(coffee));
