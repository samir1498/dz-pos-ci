...
import { DocumentsScreen } from "../../src/routes/documents";
...
describe("how the money goes back", () => {
  ...
  test("the avoir dialog shows no stored figures, because they are not its amount", async () => {
    // An avoir hands back its own `net_to_pay`, which carries no stamp and
    // depends on the quantities typed above it. The facture's 3 030,00 and
    // its 30,00 stamp are the wrong two numbers from the first avoir
    // onwards, so the dialog says in words what comes back instead.
    list = [cashFacture, ticket];
    const user = userEvent.setup();
    mount();
    await screen.findByText("FA-2026-000010");
    ...
    expect(screen.queryByTestId("refund-figures")).toBeNull();
    expect(screen.queryByTestId("refund-paid-in")).toBeNull();
    expect(screen.queryByTestId("refund-stamp-kept")).toBeNull();
    expect(screen.getByTestId("refund-avoir-note").textContent).toBe(
      fr.documents_refund_avoir_amount,
    );
  });

  test("the cancel dialog on the same facture still shows them", async () => {
    // The pair to the case above: a cancellation hands back the paper on the
    // screen, so its own stored totals are the right figures and the block
    // is not simply gone from the app.
    list = [cashFacture, ticket];
    const user = userEvent.setup();
    mount();
    await screen.findByText("FA-2026-000010");
    ...
    expect(screen.getByTestId("refund-figures")).toBeTruthy();
    expect(screen.queryByTestId("refund-avoir-note")).toBeNull();
  });

  test("cancelling a cash sale on the account leaves the field off the body", async () => {
    // The cancel dialog's own account arm, on a paper that names a customer
    // so the choice really is between notes and the ledger. The avoir case
    // above covers the same choice through the other dialog; this one is
    // what proves `cancelSale` does not carry a `refund` nobody asked for.
    list = [{ ...cashFacture, customer_id: 3 }, ticket];
    const user = userEvent.setup();
    mount();
    await screen.findByText("FA-2026-000010");
    await user.click(screen.getByRole("button", { name: "FA-2026-000010" }));
    ...
    // The notes were on offer and were not taken: the account is where the
    // dialog stands, and this paper names somebody to credit.
    expect(screen.getByRole("radio", { name: fr.documents_refund_cash })).toBeTruthy();
    expect(screen.getByRole("radio", { name: fr.documents_refund_account })).toBeTruthy();

    await user.type(screen.getByLabelText(fr.documents_reason), "erreur de facturation");
    await user.click(screen.getByRole("button", { name: fr.documents_cancel_confirm }));
