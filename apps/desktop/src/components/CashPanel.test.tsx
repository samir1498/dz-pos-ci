...
/** One month's answer as the API sends it. Every figure is written here by
 *  hand: this fixture is not asked to be arithmetically consistent with
 *  itself, because a panel that quietly re-derived `total_centimes` from the
 *  lines above it would pass a fixture that was, and fail this one. The
 *  figures are deliberately three different numbers.
 *
...
describe("the cash panel", () => {
  test("shows the cash a reversal handed back, straight off the answer", () => {
    mount();
    // 45 000 c written as the shop reads it. The separator is the narrow
    // no-break space `formatCentimes` groups with, spelled as its escape.
  ...
  test("shows the totals the core summed rather than adding the lines up itself", () => {
    // The outgoings above come to 325 000 c and the fixture says so, but the
    // three figures around it do not add up to their own totals on purpose:
    // 1 200 000 + 300 000 is 1 500 000 only because the stamp is inside the
    // sales line (the panel's own comment), and `cash_centimes` is the core's
    // subtraction. A panel that summed anything would disagree with one of
    // the three.
    mount();
    expect(screen.getByTestId("cash-in-total").textContent).toBe("15 000,00");
    expect(screen.getByTestId("cash-out-total").textContent).toBe("3 250,00");
    expect(screen.getByTestId("cash-net").textContent).toBe("11 750,00");
  });
