...
| Droit de timbre, exemption below 300 DA | art. 100-I read with circular 14/2025: "les sommes n'excédant pas 300 DA ne donnent lieu à aucun droit" | none; a 100 DA cash sale still pays the 5 DA floor | nothing at or below 300 DA |
...
| Droit de timbre, per-store toggle | no text lets a seller opt out of a duty | toggleable per store in settings | not a toggle. The regime setting decides TVA; the stamp follows the payment mode |
| TVA rate assignment | CTCA 2026 art. 21 (19 %) and art. 23 (9 %, listed by customs tariff line) | one `currentTVAPercentage` for the whole cart, set in store settings | rate per product, defaulted from the category. Open decision 3, settled 2026-09-08. Row "TVA rates", fixture `tva_rates_table` |
| TVA arithmetic | no text prescribes how a facture rounds. CTCA art. 80bis sends the tax return to CIDTA art. 324, which is about the G50, not the document | float multiply, printed with `toFixed(2)` | integer centimes, one rounding per rate group on the group's HT subtotal, half away from zero. Row "Rounding", fixture `tva_rounding_once_per_rate` |
...
| Global discount split across rate groups | no text | not applicable, one rate | **not specified in `docs/features.md`.** The totals table defines `discount` against `total_ht` and `tva` per rate group, and never says how one global discount is allocated when two rate groups are present. Question 7 of the comptable page |
| Amount in words | décret 05-468 art. 3: "prix total toutes taxes comprises, libellé en chiffres et en lettres" | own `numberToWords.js`, fr / ar / en, dinars and centimes, float input split with `Math.floor` and `Math.round` | three generators over integer centimes, fixture `words_{fr,ar,en}_golden`. The float split is exactly the bug class the money contract forbids |
...
One difference worth naming: Lumina computes `TOTAL_IN_WORDS` and prints it
next to `NET_TO_PAY`, which includes the stamp. Décret 05-468 art. 3 asks for
the "prix total toutes taxes comprises" in words. Whether the droit de timbre
belongs inside that figure is a question for the comptable, and
`docs/features.md` currently takes the same position as Lumina by defining
`amount_in_words` on `net_to_pay`.
...
| Cancellation | art. 10: the cancelled facture keeps its number and carries "facture annulée" written diagonally, Arabic "فاتورة ملغاة" | `credit_note` template ships, no cancellation overlay seen | a cancelled facture gets an avoir and is not deleted. **The diagonal overlay is not in `docs/features.md` §4** |
| Electronic issue | art. 10 allows "sous forme dématérialisée à travers le recours à un procédé informatique"; art. 4 still requires the cachet humide unless art. 11's télématique route applies, and that route waits on a joint arrêté | prints, with stamp and signature blocks | print engine renders HTML in the core, stamp and signature blocks on the layout |
| Bon de livraison and facture récapitulative | loi 04-02 art. 11 and décret 05-468 arts. 14 to 17: only for repeated regular sales to the same client, on prior authorisation from the commerce administration, with a monthly récapitulative referencing the bons | `delivery_note` and `recap_invoice` templates ship in ar / en / fr | `bon_de_livraison` is a document kind in §3 and a template in §4. The authorisation precondition is not modelled anywhere |
...
| Payment receipt | Code du timbre art. 100-I taxes any "titre qui constate un paiement", so a quittance for a later cash payment is itself stampable | `payment_receipt` template | **no `quittance` kind in §3.** Question 6 of the comptable page |
...
- Three of Lumina's calculation choices are not options for us: the stamp
  ...
  two are already corrected in `docs/features.md`; the third is a row there and
  has no fixture yet.
