...
- Six clips of the real product exist. Playwright records the five desktop scenes (ring a sale, a facture to a named company, credit and a payment, the dashboard, the same till in Arabic) in French at 1920x1080; `just demo-clips` converts them into the Remotion project. The phone scene is filmed on an Android emulator: `just demo-phone` runs Maestro against Expo Go on the `dinar` AVD with `adb shell screenrecord` around it, and the sale it rings is TK-2026-001052 at 947,40 DA in the shop file. Three things the emulator needed and nobody would guess: WSL cannot reach the Windows adb server through the firewall, so Maestro runs on the Windows side; `adb reverse` targets 127.0.0.1 while WSL mirrors Metro's dual-stack socket onto [::1] only, so an IPv4 forwarder sits in front of Metro; and a pairing code lives sixty seconds, less than the flow takes to reach the field, so the flow mints its own. Left: the Remotion cut. (@demo, @mobile, @loop)
