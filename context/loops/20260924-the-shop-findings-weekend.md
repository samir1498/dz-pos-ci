...
2026-09-24 21:25: customers and debt PR #181 open, gates green. Centimes lens reviewing before merge. For Samir on Monday: (a) a downward correction now also settles the opening debt first (features.md already said corrections settle like payments; confirm); (b) an upward correction waits behind the papers, one-line flip; (c) T30's "Sous le plafond" also shows on customers with no limit or no debt, to fix in the same PR. Known gap: the opening-debt share of a payment has no allocation row, so a statement can list a later paper under an old payment once the balance goes negative (totals right).
