...
entries:
  ...
  - ref: 'arabic-on-a-cheap-thermal-head'
    status: 'active'
    note: 'Gap 4. The ticket in Arabic prints boxes on a cheap head because no single-byte table holds Arabic; the fix draws the same ticket lines as a raster. The build-or-buy page is written (rustybuzz, ab_glyph, unicode-bidi; Noto Naskh and Plex Mono vendored); the raster writer, the preference and the 80 mm facture are what is left'
