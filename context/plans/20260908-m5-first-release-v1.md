...
tasks:
  ...
  - id: 'T4'
    ...
    status: 'done'
  - id: 'T5'
    desc: 'The Arabic and RTL polish pass over every screen and every printed document: mirrored layouts, the numerals the spec asks for, dates and money reading right, the lock and sign-in screens included because they were the last built. What can be proven here is that nothing is clipped, reversed or left in French; what cannot is whether the wording is right, which is release gate R6 and needs a native speaker. The half a reader can do without a browser is done on 2026-09-12 (PR #53): five icons that point somewhere now mirror, four hand-rolled number formats go through the shared formatter, the dialog close button and a failed Excel write read the dictionary, and the physical-side guard in `kit.test.tsx` reads one class at a time with its variants stripped, which is what caught the vertical tabs'' bar on the physical right. All six printed templates and both sign-in screens were already clean. What is left is the browser half: clipping, overflow and a glyph mirrored wrongly, which needs a machine with disk. The browser half is done on 2026-09-12 by reading the fifteen Arabic screenshots the e2e suite commits and had never been read: the keypad''s grid flowed from the page''s side so the 1 sat where a thumb reaches for the 3, the till''s credit-limit pair printed the two figures the wrong way round, the Excel picker drew a button in the language Windows was in, and a phone on the customer fiche printed backwards. The printed documents are done the same day and were worse: twenty-four values on the committed Arabic goldens laid out in an order no reader can use. The six templates isolate each value in the span it already sat in and point that span left to right; text the shop typed goes in a `bdi`. `crates/core/tests/print_bidi.rs` runs the bidi algorithm over all forty-four fixture pages rather than reading the markup for a rule, and its third assertion refuses any text carrying a digit that sits on an Arabic page with nothing isolating it. What is left is the wording, which is release gate R6 and somebody else''s.'
    status: 'done'
  - id: 'T6'
  ...
  - id: 'T10'
    ...
    status: 'done'
  - id: 'T11'
    desc: 'The two kinds of copy that sit beside the shop file, the one taken before a restore and the one taken before an upgrade, are restorable from the settings screen. Today the restore route accepts the name of a daily copy and no other kind, so both are a file swap by hand. Needs a third list on the backups route, wording in three languages and a screen change. The half of this that needed no screen is done on 2026-09-12 (PR #50): the reopen inside a restore went through `db::open` and migrated an older copy where it stood, and it goes through `open_and_upgrade` now, so that copy is copied first and the copy is the kind no prune reaches. What is left is the screen, and it grew a row: a pre-upgrade copy is listed nowhere, so an owner finds one by knowing the naming convention, and a restore can now leave one. The screen half is done on 2026-09-12: a third list on the backups panel, the restore button and its confirmation on all three lists, and a restore route that picks its folder from the shape of the name rather than from the caller. On main 2026-09-12 (PR #54). The review of it moved the confirmation from naming a time to naming which of the three lists the copy came from, because three lists can each hold a copy from the same minute and one of the three rolls the file to an older shape. Raised by the data-safety review, 2026-09-11.'
    status: 'done'
  - id: 'T12'
