...
tasks:
  ...
  - id: 'T3'
    ...
    status: 'done'
  - id: 'T4'
    desc: 'Settings as a rail of rooms instead of one scroll, each its own route; the phones room mints its code on arrival; a Phones entry in the sidebar gated on edit_settings so pairing is reachable without hunting'
    status: 'done'
acceptance:
