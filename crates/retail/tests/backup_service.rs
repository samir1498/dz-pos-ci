...
use diesel::prelude::*;
use dzpos_kernel::services::backup;
...
fn the_copy_opens_and_holds_the_same_rows() {
    ...
    let mut copy = dzpos_retail::db::open(&made.path).unwrap();
    ...
    let summary = backup::verify(&made.path, dzpos_retail::shop_counts::for_verify).unwrap();
...
fn a_file_that_is_not_a_database_or_is_torn_fails_verify() {
    ...
    let torn = backup::verify(&made.path, dzpos_retail::shop_counts::for_verify).unwrap_err();
    ...
    let not_a_db = backup::verify(&junk, dzpos_retail::shop_counts::for_verify).unwrap_err();
    ...
    assert_eq!(
        backup::verify(&missing, dzpos_retail::shop_counts::for_verify)
            .unwrap_err()
            .code(),
        "validation"
    );
...
fn a_copy_from_a_newer_app_version_is_refused() {
    ...
    let refused = backup::verify(&made.path, dzpos_retail::shop_counts::for_verify).unwrap_err();
