...
use dzpos_retail::services::{cancellation, debt, documents, products, stock};
...
fn a_line_cannot_be_credited_past_what_earlier_avoirs_left_on_it() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "qty_milli"),
        "{err:?}"
    );
...
fn the_running_total_of_avoirs_never_passes_what_the_facture_asked_for() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "lines"),
        "{err:?}"
    );
    assert_eq!(
        documents::list(&mut conn, SHOP, Some(DocumentKind::Avoir))
...
fn only_a_facture_that_still_stands_is_credited() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::NotFound { entity, .. }) if entity == "facture"),
        "{err:?}"
    );
...
fn an_avoir_that_credits_nothing_is_refused() {
    ...
    .unwrap_err();
    assert!(
        matches!(zero, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "qty_milli"),
        "{zero:?}"
    );
}
...
fn a_line_of_another_facture_cannot_be_credited_on_this_one() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::NotFound { entity, .. }) if entity == "document_line"),
        "{err:?}"
    );
}
