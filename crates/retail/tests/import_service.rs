...
use dzpos_retail::services::import::{self, Outcome};
...
fn apply_writes_nothing_at_all_when_one_row_is_refused() {
    ...
    match refused {
        dzpos_retail::error::RetailError::Kernel(CoreError::Validation { field, .. }) => {
    ...
    assert!(products::list(&mut conn, SHOP).unwrap().is_empty());
    let categories = dzpos_retail::services::categories::list(&mut conn, SHOP).unwrap();
    assert!(
