...
use dzpos_kernel::services::clock::Month;
...
fn writing_an_expense_leaves_the_audit_entry_its_own_change_writes() {
    ...
    let entry = dzpos_kernel::services::audit::list(&mut conn, SHOP)
        .unwrap()
        .into_iter()
...
fn field_of<T>(result: Result<T, dzpos_retail::error::CoreError>) -> Option<String> {
    match result {
        Err(dzpos_retail::error::CoreError::Validation { field, .. }) => Some(field),
        _ => None,
    }
}
