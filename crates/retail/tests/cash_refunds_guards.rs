...
use dzpos_retail::services::cash_refunds::Refund;
use dzpos_retail::services::debt::PaymentMethod;
use dzpos_retail::services::shifts::NewShift;
use dzpos_retail::services::{cancellation, cash, debt, documents, products, shifts};

mod common;
use common::cash_refunds::{
    a_customer_in, a_facture, a_facture_in, a_second_shop, a_second_shop_selling_factures,
    an_anonymous_cash_facture, at, day, last_after, line, product, product_in,
};
...
/// One shop's payments never bound another shop's refund.
///
/// The credit arm of `repos::cash_refunds::still_to_hand_back` joins
/// `debt_allocations` to `debt_ledger` and filters `shop_id` on both. Nothing
/// stood on the ledger side of that filter: `a_facture` hardcodes shop 1, so
/// every credit facture in either suite lived there and a query reading a
/// second shop's payments would have agreed with every assertion in the file.
///
/// Here both shops carry a credit facture for the same customer name and the
/// same goods, and shop 2's is paid four times as much. Shop 1's paper was
/// paid 1 000,00, so 1 000,00 is what may go back in notes; asking for 2 units
/// is refused. Under an unscoped ledger read shop 2's 4 000,00 would be in
/// reach and that ask would go through.
#[test]
fn a_second_shops_payments_do_not_widen_what_this_shop_hands_back() {
    let (_dir, mut conn) = open_temp_selling_factures();
    a_second_shop_selling_factures(&mut conn, 2);

    // Shop 2 first, so its rows are the older ones: a query that read the
    // wrong shop would find them without having to look past shop 1's.
    let their_product = product_in(&mut conn, 2, "Ciment", 100_000, 0);
    let their_customer = a_customer_in(&mut conn, 2, "Entreprise Benali");
    let theirs = a_facture_in(
        &mut conn,
        2,
        their_customer,
        vec![line(their_product, 4_000)],
        PaymentMode::Credit,
        14,
    );
    assert_eq!(theirs.totals.net_to_pay, Money::centimes(400_000));
    debt::pay(
        &mut conn,
        2,
        OWNER,
    ...
    // Shop 1's own paper: 4 000,00 on the account, 1 000,00 of it paid.
    let p = product(&mut conn, "Ciment", 100_000, 0);
    let c = an_identified_customer(&mut conn, "Entreprise Benali");
    let ours = a_facture(&mut conn, c, vec![line(p, 4_000)], PaymentMode::Credit, 14);
    let line_id = ours.lines[0].id;
    debt::pay(
        &mut conn,
        SHOP,
        OWNER,
    ...
    // 2 units is 2 000,00: over shop 1's 1 000,00 and well under the
    // 4 000,00 the other shop was paid.
    let err = avoir::issue_settling(
        &mut conn,
        SHOP,
        OWNER,
    ...
    // And this shop's own 1 000,00 still goes back, so the filter narrowed
    // rather than emptied the read.
    let credit = avoir::issue_settling(
        &mut conn,
        SHOP,
        OWNER,
