...
use dzpos_retail::services::dashboard;
use dzpos_retail::services::documents::Document;
use dzpos_retail::services::sales::{self, NewSale, NewSaleLine, SaleKind};
use dzpos_retail::services::{avoir, cancellation, cash, products};
...
fn a_month_of_trading(conn: &mut SqliteConnection) {
    ...
    .unwrap();
    let category = dzpos_retail::services::expenses::categories(conn, SHOP).unwrap()[0].id;
    for d in [2u32, 20] {
        dzpos_retail::services::expenses::create(
            conn,
            SHOP,
            OWNER,
            dzpos_retail::services::expenses::NewExpense {
