...
use dzpos_retail::money::{Money, PaymentMode, Regime, Totals};
use dzpos_retail::services::customers::{self, NewCustomer, PartyKind};
...
fn a_row_carrying_a_debit_and_a_credit_at_once_is_refused_by_the_check() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "debit"),
        "{err}"
    );
    assert_eq!(
        debt::balance(&mut conn, SHOP, customer).unwrap(),
...
fn a_note_longer_than_a_statement_prints_is_refused() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "note"),
        "{err}"
    );
}
...
fn a_movement_pointing_at_another_shops_document_is_refused() {
    ...
    assert!(
        matches!(
            err,
            dzpos_retail::error::RetailError::Kernel(CoreError::NotFound {
                entity: "document",
                ..
            })
        ),
...
fn an_adjustment_of_nothing_is_refused_and_writes_no_row() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "amount"),
        "{err}"
    );
...
fn a_payment_of_nothing_or_of_a_negative_is_refused() {
    ...
    for centimes in [0, -1_000] {
        ...
        .unwrap_err();
        assert!(
            matches!(refused, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "amount_centimes"),
...
fn a_statement_opens_at_what_was_owed_before_the_range_and_closes_at_the_last_movement_in_it() {
    ...
    assert_eq!(
        ...
        Some(dzpos_retail::services::documents::DocumentKind::Facture)
    );
}
...
fn a_movement_is_stamped_by_the_shops_clock_and_not_by_utc() {
    ...
    let drift = written
        .created_at
        .signed_duration_since(dzpos_kernel::services::clock::now())
        .num_seconds()
        .abs();
    assert!(
        drift <= 5,
