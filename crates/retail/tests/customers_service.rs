...
fn a_negative_opening_debt_is_refused_and_no_fiche_is_left_behind() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "opening_debt"),
        "{err}"
    );
...
fn another_shop_gets_not_found_on_a_read_and_on_an_update() {
    ...
    let refused = customers::prove(&mut conn, 2, made.id).unwrap_err();
    assert!(
        matches!(refused, CoreError::NotFound { entity, .. } if entity == "customer"),
        "{refused:?} is not the refusal the neighbour's fiche should get"
    );
...
fn a_field_longer_than_a_facture_prints_is_refused() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "address"),
        "{err}"
    );
...
fn a_fiche_with_no_name_is_refused_and_a_blank_identifier_is_stored_as_nothing() {
    ...
    assert!(
        matches!(err, dzpos_retail::error::RetailError::Kernel(CoreError::Validation { ref field, .. }) if field == "name"),
        "{err}"
    );
