...
use crate::models::purchase::{
    purchase_series, PurchaseLineRowWrite, PurchaseReceiptLineRowWrite, PurchaseReceiptRowWrite,
    PurchaseRowWrite,
};
use crate::money::Money;
use crate::repos::{counters, purchases as repo};
use crate::services::purchase_account::{self, PurchaseAccount};
use crate::services::purchase_landed::{spread, step_value};
...
pub use crate::services::purchase_landed::NewLine;
...
pub struct PurchaseView {
    ...
    /// Total, arrived, paid and still owed (`services::purchase_account`).
    pub account: PurchaseAccount,
}
...
pub fn get(conn: &mut SqliteConnection, shop_id: i32, id: i32) -> Result<PurchaseView, CoreError> {
    ...
    let account = purchase_account::of(conn, shop_id, &purchase, &lines)?;
    Ok(PurchaseView {
        purchase,
        lines,
        receipts,
...
pub fn save(
    ...
    conn.transaction(|conn| {
        ...
        let landed = spread(conn, shop_id, &new.lines, new.transport, new.extra_costs)?;
        ...
        // The number is taken last before the row, inside the transaction:
        // anything refused from here on rolls the counter back with the
        // order, so no refusal burns one (features.md §1, Purchase).
        let number = counters::take_next(conn, shop_id, purchase_series(day.year()))?;
        ...
        audit::record(
            ...
            audit::Change {
                ...
                after: Some(
                    serde_json::json!({
                        "supplier_id": new.supplier_id,
                        "number": purchase.printed_number(),
