...
fn a_checkpoint_empties_the_log_and_says_so_when_it_could_not() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("t.db");
    let mut till = dzpos_core::db::open(&path).unwrap();
    diesel::sql_query(
    ...
    dzpos_core::db::checkpoint(&mut till).unwrap();
    ...
    let mut reader = dzpos_core::db::open(&path).unwrap();
    ...
    let refused = dzpos_core::db::checkpoint(&mut till).unwrap_err();
    assert!(
