...
use dzpos_core::money::{Bps, Regime};
...
// --- discount threshold: the dated series T1 puts beside the régime's. ---
...
fn a_shop_that_never_set_one_reads_zero() {
    ...
    let (_dir, mut conn) = open_temp();
    assert_eq!(
        settings::discount_threshold_as_of(&mut conn, SHOP, at(2026, 3, 15)).unwrap(),
        Bps::ZERO
    );
    assert_eq!(
        settings::discount_threshold_current(&mut conn, SHOP, at(2026, 3, 15)).unwrap(),
        None
    );
    assert_eq!(
        settings::discount_threshold_planned(&mut conn, SHOP, at(2026, 3, 15)).unwrap(),
        None
    );
}
...
fn a_date_between_two_threshold_rows_reads_the_earlier_one() {
    let (_dir, mut conn) = open_temp();
    settings::set_discount_threshold(
        &mut conn,
        SHOP,
        OWNER,
        Bps::new(500).unwrap(),
    ...
    assert_eq!(
        settings::discount_threshold_as_of(&mut conn, SHOP, at(2026, 9, 1)).unwrap(),
        Bps::new(1000).unwrap()
    );
}
...
fn the_current_threshold_carries_the_date_it_took_effect() {
    ...
    .unwrap();
    let current = settings::discount_threshold_current(&mut conn, SHOP, at(2026, 3, 15))
        .unwrap()
        .unwrap();
...
fn a_threshold_change_dated_in_the_future_is_planned_not_current() {
    ...
    assert_eq!(
        settings::discount_threshold_as_of(&mut conn, SHOP, today).unwrap(),
        Bps::ZERO,
        "the future row does not apply yet, and nothing else was ever set"
    );
    let planned = settings::discount_threshold_planned(&mut conn, SHOP, today)
        .unwrap()
        .unwrap();
    assert_eq!(
...
fn the_threshold_series_is_scoped_by_shop() {
    ...
    .unwrap();
    assert_eq!(
        settings::discount_threshold_as_of(&mut conn, 2, at(2026, 3, 15)).unwrap(),
        Bps::ZERO,
...
fn a_change_to_the_threshold_already_in_force_on_that_day_is_refused() {
    ...
    .unwrap();
    let err = settings::set_discount_threshold(
        &mut conn,
        SHOP,
        OWNER,
        Bps::new(500).unwrap(),
