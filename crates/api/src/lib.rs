...
/// it.
pub struct Restored {
    /// Retail-only (S5): a kernel-only build has no shop counts to read a
    /// restored copy for, so `verify` below asks nothing of it and this
    /// carries none.
    #[cfg(feature = "retail")]
...
impl AppState {
    ...
    pub fn support_bundle(&self, actor_id: i32) -> Result<Vec<u8>, ApiError> {
        ...
        self.with_conn(|conn| -> Result<Vec<u8>, CoreError> {
            ...
            // Retail-only (S5): a kernel-only build has no shop counts to
            // add, so `counts.txt` carries none rather than one made up.
            #[cfg(not(feature = "retail"))]
            let retail_counts: [(&'static str, i64); 0] = [];
    ...
    pub fn restore(&self, backup_path: &Path, actor_id: i32) -> Result<Restored, ApiError> {
        ...
        // Retail-only (S5): `verify` still opens the copy and runs SQLite's
        // integrity check either way (`Restored`'s own doc says why a
        // kernel-only build reads none of its counts), it just has nothing
        // left to count once it has. Named `()` and carried through like
        // every other build's counts, rather than inlined at each of the
        // three places `summary` is read below.
        #[cfg(not(feature = "retail"))]
        #[allow(clippy::let_unit_value)]
        let summary = backup::verify(backup_path, |_| Ok(())).map_err(ApiError::Request)?;
