...
//! `GET /users` is the sixth and the M4 T8 one: who the shop's staff are is
//! the owner's read alone (`services::permissions::can`'s own doc), not an
//! ordinary list a cashier is already looking at.
//!
