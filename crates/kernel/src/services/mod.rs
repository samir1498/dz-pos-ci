...
/// holding a space.
///
/// `pub`, not `pub(crate)`: nine of its ten callers are in `dzpos-retail`
/// since the kernel crate split (S3 of
/// `a-kernel-crate-and-retail-as-the-first-module`), which cannot see a
/// `pub(crate)` item across the crate boundary.
pub fn optional_field(field: &str, value: Option<&str>) -> Result<Option<String>, CoreError> {
...
///
/// `pub`, not `pub(crate)`: both callers, `sales` and `shifts`, are in
/// `dzpos-retail` since the kernel crate split.
pub fn role_of(
...
/// `pub`, not `pub(crate)`: three of its five callers are in `dzpos-retail`
/// since the kernel crate split.
pub fn bounded_field(field: &str, value: &str) -> Result<(), CoreError> {
