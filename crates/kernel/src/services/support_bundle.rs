...
/// else may be in the archive. The walking test spells these six names out
/// on its own side rather than reading them from here, so a seventh entry
/// fails that test even when it is added to this list, and whoever adds one
/// has to come past the exclusion list above to do it.
...
#[cfg(test)]
#[path = "../../tests/unit/services_support_bundle.rs"]
mod tests;
