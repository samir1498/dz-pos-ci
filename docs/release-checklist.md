...
Read the states as: **done** means it shipped and a test holds it.
...
| A comptable has confirmed every fiscal row | Samir, through a comptable | The rows are read off the texts and the fixtures agree with each other, which is not the same as an Algerian accountant saying they match practice. | `docs/features.md`, each row's own note (gate R8) |
...
|---|---|---|
| The article that puts NIF and AI on a facture, cited | The spec asserts the requirement and names no text for it. A claim about what an Algerian invoice must carry needs the article it comes from. | gate R3, `docs/features.md` |
