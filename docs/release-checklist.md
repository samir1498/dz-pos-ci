...
| The org can receive releases cut on the mirror | Samir | The publish job creates the release on Dinar-dz/dz-pos with `ORG_RELEASE_TOKEN`, a fine-grained PAT with contents read+write on that repo, stored as a secret on samir1498/dz-pos-ci. Until it exists the publish job fails fast with its own error. | that secret on the mirror; nothing else needs it |
