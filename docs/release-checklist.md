...
| A tag on `main` as the only thing that publishes a release | `.github/workflows/release.yml`, `.github/scripts/release-gate.sh`, `just release vX.Y.Z` (tags origin and mirror; the mirror tag run builds) | eighteen cases in that script's own test |
| A shop can ask the app to check for the next version, and see one of three answers; the update is only ever signed and installed with the shop's own yes | `apps/desktop/src-tauri/src/updater.rs`, `tauri.conf.json`'s `plugins.updater`, the About screen's `UpdateCheckPanel` | `updater_config_tests` (the pubkey is still the named placeholder, every endpoint is https), the shape test and the two manifest-`size` tests in `updater.rs`, and the screen's own tests for each answer and the install doc |
| The release workflow refuses to publish an unsigned updater manifest; a release with no signing key still ships, with no manifest. The two keys are coupled in practice: a build with a Windows code-signing certificate configured but no updater signing key stops with an error instead of shipping an installer the release would wrongly call signed, since `--no-sign` would have silently skipped both | `.github/workflows/release.yml` (`gate` job's `updater` output, `publish` job's manifest assembly and the build's "signed Windows" step) | four of the eighteen cases above, `release-gate.test.sh` §9 |
| All three installers are built on the mirror and published as one release on the org repo | `.github/workflows/release.yml` (matrix build, `publish` to Dinar-dz/dz-pos via `ORG_RELEASE_TOKEN`), stable filenames (`Dinar-Setup.exe`, `Dinar.dmg`, `Dinar.AppImage`) | dry-run dispatch builds all three as artifacts; the manifest covers every platform with a `.sig` beside it |
| The landing page offers the installer matching the visitor's system | `apps/landing/src/components/Download.astro` (OS sniffing highlights, never reorders), `lib/site.ts` download URLs | `smoke.test.ts` asserts all three stable URLs and the `data-os` markers on every route |
...
|---|---|---|---|
| A Windows code-signing certificate is bought | Samir | Without one, every customer who installs sees a warning that the software is from an unknown publisher. Buying one takes days, which is why it sits at the top of this list rather than at the end. | the release workflow's secrets; the workflow already builds unsigned and marks the release a draft when the secret is absent |
| The org can receive releases cut on the mirror | Samir | The publish job creates the release on Dinar-dz/dz-pos with `ORG_RELEASE_TOKEN`, stored as a secret on samir1498/dz-pos-ci (today Samir's own login token; a fine-grained PAT limited to the org repo is the replacement when convenient -- if `gh auth` re-login rotates it, publish fails fast with its own error). | that secret on the mirror; nothing else needs it |
| Somebody holds the key that signs updates | Samir | The key never enters the repository and the build signs with a secret. Until it exists there is no update to sign, and until somebody owns it there is nobody to regenerate it if it leaks. | `docs/architecture.md` § Release |
...
| A real printed facture from an Algerian shop has been seen | Samir | The templates are golden files. They are frozen against what we believe a facture looks like, not against one somebody has held. | `docs/features.md` open decision 4 |
| A native speaker has read the Arabic | Samir, or somebody he names | The amount in words on a facture is a legal phrase and the app writes it. The public page's Arabic needs the same read. | gate R6, and `apps/landing/src/i18n/ar.ts` |
