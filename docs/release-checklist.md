...
| Both installers are built on the mirror and published as one release on the org repo | `.github/workflows/release.yml` (matrix build, `publish` to Dinar-dz/dz-pos via `ORG_RELEASE_TOKEN`), stable filenames (`Dinar-Setup.exe`, `Dinar.AppImage`) | dry-run dispatch builds both as artifacts; the manifest covers every platform with a `.sig` beside it |
