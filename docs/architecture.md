...
| Phone or second till to the serving desktop over the shop LAN | **Decided M6 T1 (2026-09-14):** HTTP on the LAN the owner enables (`bind_lan` on `0.0.0.0`, `mDNS` `_dzpos._tcp.local.` `Dinar-<shop_id>`, Windows Firewall banner). TLS with a self-signed fingerprint in the QR stays the candidate for a shop that does not trust its Wi-Fi; plain HTTP on a trusted shop Wi-Fi is the v1. Decision is reversible: the QR already carries `shop`+`v`, adding `fp` later does not change the flow. | a device token: the QR carries a 60-second single-use pairing token, the phone trades it for a long-lived device token stored on the phone, revocable from settings; every request shows it the way the webview shows the launch token | M6 T1, `crates/api/src/mdns.rs`, `crates/api/src/lib.rs::bind_lan` |
