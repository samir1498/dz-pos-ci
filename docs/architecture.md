...
| Phone or second till to the serving desktop over the shop LAN | **Decided M6 T1 (2026-09-14), proven M7 (2026-09-15):** HTTP on the LAN the owner enables (`bind_lan` on `0.0.0.0`, `mDNS` `_dzpos._tcp.local.` `Dinar-<shop_id>`, Windows Firewall banner). TLS with a self-signed fingerprint in the QR stays the candidate for a shop that does not trust its Wi-Fi; plain HTTP on a trusted shop Wi-Fi is the v1. The QR already carries `shop`+`v`, adding `fp` later does not change the flow. | three credentials, each one header: the launch token (`Authorization: Bearer`, per-run, worthless after `just api` ends), a device token (`X-Dzpos-Device`, 60-second single-use QR traded for a long-lived token, stored via `AsyncStorage` on the phone, revocable from settings, checked off-loopback in `crates/api/src/device.rs` with `ConnectInfo` and `BEGIN IMMEDIATE` on the claim race, `revoked` is `401` before session), and a session token (`X-Dzpos-Session` or httpOnly cookie, the person). `queue.ts` retries the same `idempotency_key` (`sale_idempotency_keys` dedupe, `200` replay) and skips another signer's queued items; a retried ring also prints. | M6 T1, `crates/api/src/mdns.rs`, `crates/api/src/lib.rs::bind_lan`; M7 T1–T4 `crates/api/src/device.rs`, `crates/core/src/services/pairing.rs`, `crates/core/src/services/sales.rs`, `apps/mobile/src/lib/session.ts` |
