...
outgoing on the day it was handed over, by the person who handed it: an avoir
against a facture that has been paid for, or a cancelled cash sale, which is
where an anonymous customer's refund lands because there is no ledger to
credit. "Paid for" is the money actually received against that facture and
not the facture whole, so a credit facture half paid hands back up to that
half and no more, and one nobody has paid anything on hands back nothing. A
card sale counts as paid in and may be handed back in notes: the card takings
keep the sale and the cash outgoing says where the notes went
(`a_card_facture_credited_in_cash_hands_back_the_whole_of_what_the_card_took`).
A reversal settled on the ledger moves no notes and appears here as nothing.
A sale refunded in cash stays in the takings of the day it was sold, because
the drawer did take that money, while its refund lands on the day the notes
went back: a ticket rung up on Monday and handed back on Wednesday is in
Monday's takings and out of Wednesday's, and over the month the two come to
nothing. Only a cancellation that handed nothing back leaves the day it was
sold on. The fiscal rules table gains no row for any of this, because nothing
here changes what a document charges.
...
| Cash handed back | a reversal settled in notes gives back what the customer handed over less the droit de timbre: a cancelled cash sale refunds `net_to_pay - stamp`, and an avoir refunds its own `net_to_pay`, which carries no stamp to begin with. What may go back is bounded by what actually came in against the paper, less whatever has already gone back on it: a cash or card sale was settled when it was issued, so the whole `net_to_pay` came in, and a credit sale put the money on an account, so what came in is the payments allocated to it and a half-paid facture hands back that half and no more. A card sale counts as paid in and may be handed back in notes, because a shop settling a returned card sale out of the drawer is ordinary: the card takings keep the sale and the cash outgoing says where the notes went (`a_card_facture_credited_in_cash_hands_back_the_whole_of_what_the_card_took`). The bound is never `remaining_debt`, which a downward adjustment zeroes with nothing coming in, so a written-off facture would otherwise read as paid for. An assumption, the same reading the avoir row below takes one step further: the Code du timbre taxes the payment and says nothing about notes going back over the counter, so keeping the stamp on a reversal is a reading to confirm with the comptable (R8). A reversal settled on a customer's ledger hands over no notes and records none | `a_cancelled_cash_sale_hands_back_everything_except_the_stamp`, `an_avoir_settled_in_cash_hands_back_its_own_figure_which_carries_no_stamp`, `a_ticket_refunded_on_wednesday_stays_in_mondays_takings_and_the_month_nets_to_nothing`, `a_card_facture_credited_in_cash_hands_back_the_whole_of_what_the_card_took`, `a_credit_facture_part_paid_is_refunded_in_cash_up_to_what_came_in`, `a_facture_written_off_by_an_adjustment_is_not_refunded_in_cash` | Code du timbre 2026 art. 100-I for the stamp; the rest is a design choice, not law |
...
- Thermal: the 80 mm ticket is HTML and ESC/POS, both golden-filed in three
  ...
  **text for French and English, a raster for Arabic**. Which path a shop's
  ...
  - Raster path: no codepage at all. There is no single-byte table with
    ...
    Digits stay Western, and each Latin stretch of an Arabic line — an
    amount, a rate, a date, a phone number, a street with a number on it —
    is handed to the bidi algorithm inside an isolate (`U+2066 … U+2069`),
    the same fix the HTML ticket took. Without it the algorithm reads a
    number as right-to-left when it decides who the space or sign beside it
    belongs to, and the paper says `10,00-` for a discount and `% 19` for a
    rate. The goldens are pictures a reviewer opens — `ar.png`,
    `ar-ifu.png`, `ar-card.png`, `ar-credit.png`, `ar-credit-held.png`,
    the last two carrying the three rows of the debt block — decoded back
    out of the bytes the head is sent, and the dump names a band
    `<raster 576x1184>`. A line
