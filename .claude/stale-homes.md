...
| How many permissions there are, and which role holds each | `crates/core/src/services/permissions.rs` (`Permission::ALL` for the count, `can` for who holds what) | `docs/features.md` §5 restates both in prose and moves with them; `crates/api/src/dto/auth.rs` and `packages/shared/src/schemas/session.ts` mirror the list and the compiler holds them to it |
